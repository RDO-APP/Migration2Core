...
namespace rdoappProject.Api.Models
{
    public class TarefaModel
    {
        ...
        public static byte[] RelatorioControleHorasEquipamentoTarefa(RelatorioControleHorasEquipamentoTarefaViewModel filter)
        {
            ...
            string periodo = string.Format("{0:dd/MM/yyyy HH:mm:ss} A {1:dd/MM/yyyy HH:mm:ss}", (DateTime)filter.DataInicial, (DateTime)filter.DataFinal);
        ...
        public static int SalvarLaudo(dynamic param)
        {
            using (var context = new rdoappEntities())
            {
                try
                {
                    ...
                    // Verificar se já existe laudo para esta data e obra
                    // Usar DbFunctions.TruncateTime para comparar apenas a data
                    ...
                    // Configurar dados do laudo
                    _laudo.lau_dt_laudo = dataLaudo;
                    _laudo.lau_id_status = 1; // Status inicial
                    _laudo.lau_id_obra = idObra;
                    _laudo.lau_id_colaborador = idColaborador;
                    _laudo.lau_dt_geracao = DateTime.Now;

                    // Converter valores dos campos seguindo a estrutura atual da entidade
                    // Campos que são bool na entidade atual
                    ...
                    _laudo.lau_tp_fundo = param.fundo ?? false;
                    _laudo.lau_tp_nivel_cloro_2 = param.nivelCloro2 ?? false;
                    _laudo.lau_tp_nivel_bacterias = param.bacterias ?? false;
                    _laudo.lau_tp_nivel_proliferacao = param.proliferacao ?? false;
                    
                    // Campo que é int na entidade atual
                    _laudo.lau_tp_alcalinidade = param.NivelAlcalinidade ?? 0;

                    // Salvar ou atualizar laudo
                    if (_laudo.lau_id_laudo > 0)
                    {
        ...
        private static bool ConvertSimNaoToBool(dynamic value)
        {
            if (value == null) return false;
            string strValue = value.ToString().ToLower();
            return strValue == "sim" || strValue == "true" || strValue == "1";
        }
    ...
    public class HistoricoTarefaViewModel
    {
        ...
        public HistoricoTarefaViewModel(tarefa item)
        {
            ...
            this.CodigoParalizacao = item.tar_codigo_paralizacao != null ? item.tar_codigo_paralizacao.ToString() : string.Empty;
