...
// Missing filters - Added to fix Angular errors
angular.module('app').filter('lookup', function() {
    return function(input, lookup) {
        if (!input || !lookup || !Array.isArray(lookup)) return '-';
        
        // Find the item in the lookup array by id
...
angular.module('app').filter('simNao', function() {
    return function(input) {
        if (input === 'sim' || input === 'Sim' || input === 'SIM') return 'Sim';
        if (input === 'nao' || input === 'não' || input === 'Não' || input === 'NAO' || input === 'NÃO') return 'Não';
        return input || '-';
    };
});