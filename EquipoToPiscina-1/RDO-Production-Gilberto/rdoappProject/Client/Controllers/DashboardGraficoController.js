    ...
    function DashboardGraficoController($http, $location, Auth, $rootScope, $scope) {
        ...
        this.carregarDashboard = function () {
            ...
            $http({
                ...
                controller.bacteriasSim = data.filter(x => x.lau_tp_nivel_detritos === true).length;
                controller.bacteriasNao = data.filter(x => x.lau_tp_nivel_detritos === false).length;

                controller.lau_tp_alcalinidade = data.select(x = x.lau_tp_alcalinidade).lau_tp_alcalinidade
