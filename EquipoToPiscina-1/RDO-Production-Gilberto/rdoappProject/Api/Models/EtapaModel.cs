    ...
    public class EtapaModel
    {
        ...
        public static List<EtapaViewModel> ObterEtapaTarefa(EtapaViewModel filter)
        {
            ...
            foreach (var item in listTemp)
            {
                ...
                // FIX: Group by tar_id_tarefa instead of tar_nr_agrupador to prevent card duplication
                tarefas = tarefas.OrderByDescending(t => t.tar_id_tarefa).ThenByDescending(t => t.tar_dt_medicao).ThenByDescending(t => t.tar_dt_medicao_hora_final).GroupBy(t => t.tar_id_tarefa).Select(g => g.FirstOrDefault()).Where(t => t != null).ToList();
                ...
                if (tarefas.Count > 0)
                {
                    foreach (var t in tarefas)
                    {
                        ...
                        listTarefa.Add(new TarefaViewModel
                        {
                            ...
                            PrimeiraExecucao = TarefaModel._ObterPrimeiroDiaExecutado(t.tar_id_tarefa),
                            UltimaExecucao = TarefaModel._ObterUltimoDiaExecutado(t.tar_id_tarefa),
