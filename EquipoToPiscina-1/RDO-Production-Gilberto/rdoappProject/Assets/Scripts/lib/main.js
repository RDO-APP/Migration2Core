...
function validate(form) {
    ...
    if ($form.length !== 1) {
        console.error("Formulário não encontrado ou múltiplos encontrados.", $form);
        return false;
    }

    var validation = $form.data('validator') || $form.validate();

    if (!validation || typeof validation.checkForm !== 'function') {
        console.error("Plugin de validação não inicializado corretamente.", validation);
        return false;
    }

    if (validation.checkForm() === false) {
        const err = new Error();
        console.warn("form has errors! Linha:", (err.stack || "").split("\n")[1]);
...
//    console.log("Length do form:", $(form).length);
//    if (validation.checkForm() == false) {
//        const err = new Error();
...
//        console.log("Erro chamado em: " + callerLine.trim());
