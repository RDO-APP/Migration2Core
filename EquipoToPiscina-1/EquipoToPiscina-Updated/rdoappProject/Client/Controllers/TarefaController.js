(function () {
    ...
    function TarefaController($scope, $http, $location, ViewBag, Auth, $rootScope, Validacao, Convert, Download, Permission, Email, $timeout) {
        ...
        this.validarCamposRelatorioHorasEquipamento = function () {
         ...
         this.gerarRelatorioHorasEquipamentoExcel = function () {
            this.gerarRelatorioHorasEquipamento('Excel');
        }

        this.gerarRelatorioHorasEquipamentoPDF = function () {
            this.gerarRelatorioHorasEquipamento('PDF');
        }

        this.gerarRelatorioHorasEquipamento = function (tipoRelatorio) {
            if (this.validarCamposRelatorioHorasEquipamento()) {
                ...
                $http({
                    ...
                    let nomeRelatorio = "Relatório Controle de Horas: " + Auth.getUser().obra.descricao + "__" + filter.DescricaoTarefa + "__" + filter.dataInicial.toLocaleDateString('pt-BR').replaceAll('/', '-') + "__até__" + filter.dataFinal.toLocaleDateString('pt-BR').replaceAll('/', '-') + (tipoRelatorio == "PDF" ? ".pdf" : ".xls");
